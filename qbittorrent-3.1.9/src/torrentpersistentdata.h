...
public:
  ...
  static void saveTorrentPersistentData(const QTorrentHandle &h, const QString &save_path = QString::null, const bool is_magnet = false, const QString& path = QString::null) {
    Q_ASSERT(h.is_valid());
  ...
  static QString getTorrentFilePath(QString hash) {
      QIniSettings settings(QString::fromUtf8("qBittorrent"), QString::fromUtf8("qBittorrent-resume"));
      const QHash<QString, QVariant> all_data = settings.value("torrents").toHash();
      const QHash<QString, QVariant> data = all_data.value(hash).toHash();
      return data.value("torrent_path").toString();
    }
