...
options_imp::options_imp(QWidget *parent):
  ...
  connect(dbPath, SIGNAL(textChanged(QString)), SLOT(enableApplyButton()));
  connect(dbPathBackup, SIGNAL(textChanged(QString)), SLOT(enableApplyButton()));
  connect(firstBackup, SIGNAL(timeChanged(QTime)), SLOT(enableApplyButton()));
  connect(secondBackup, SIGNAL(timeChanged(QTime)), SLOT(enableApplyButton()));
  connect(makeBackup, SIGNAL(toggled(bool)), SLOT(enableApplyButton()));
  connect(redownloadActive, SIGNAL(toggled(bool)), SLOT(enableApplyButton()));
...
#ifdef BASEPROTECT
  connect(browseButton, SIGNAL(clicked()), this, SLOT(browseForDB()));
  connect(backupBrowseButton, SIGNAL(clicked()), this, SLOT(browseForBackup()));
#endif
...
#ifdef BASEPROTECT
void options_imp::browseForDB(){
    QString dir = QFileDialog::getExistingDirectory(this,
                                                    tr("Choose Database Directory"),
                                                    "",
                                                    QFileDialog::ShowDirsOnly | QFileDialog::DontResolveSymlinks);

    if(dir != Preferences().getBackupFolder())
    {
        dbPath->setText(dir);
        Preferences().setDBPath(dir);
    }
}

void options_imp::browseForBackup()
{
    QString dir = QFileDialog::getExistingDirectory(this,
                                                    tr("Choose Backup Directory"),
                                                    "",
                                                    QFileDialog::ShowDirsOnly | QFileDialog::DontResolveSymlinks);

    if(dir != Preferences().getDBPath())
    {
        dbPathBackup->setText(dir);
        Preferences().setBackupFolder(dir);
    }
}
#endif
...
#ifdef BASEPROTECT
  pref.setDBPath(dbPath->text());
  //pref.setBackupFolder(dbPathBackup->text());
  //pref.setMakeBackup(makeBackup->checkState() == Qt::Checked);
  pref.setRestartTorrents(redownloadActive->checkState() == Qt::Checked);
  //pref.setBackupHours(std::make_pair(firstBackup->time(), secondBackup->time()));
  //pref.setSendingHitsEnabled(sendHits->checkState() == Qt::Checked);
  //pref.setStatSerwer(bp_server->text());

  //QBtSession::instance()->resetSocket();
  //QBtSession::instance()->BPTick();
#endif
}
...
#ifdef BASEPROTECT
  dbPath->setText(pref.getDBPath());
  //dbPathBackup->setText(pref.getBackupFolder());
  redownloadActive->setChecked(Preferences().getRestartTorrents());
  //makeBackup->setChecked(Preferences().getMakeBackup());
  //firstBackup->setTime(Preferences().getBackupHours().first);
  //secondBackup->setTime(Preferences().getBackupHours().second);
#endif
}
