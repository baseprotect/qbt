...
public:
  ...
  {
    qDebug() << "Preferences constructor1";
  }
  ...
  void setDBPath(QString path){
      setValue(QString::fromUtf8("Preferences/Baseprotect/DatabasePath"), path);
  }

  QString getDBPath() const{
      return value(QString::fromUtf8("Preferences/Baseprotect/DatabasePath"), "").toString();
  }

  QString getBackupFolder() const{
      return value(QString::fromUtf8("Preferences/Baseprotect/BackupFolder"), "").toString();
  }

  void setBackupFolder(QString folder){
      setValue(QString::fromUtf8("Preferences/Baseprotect/BackupFolder"), folder);
  }

  std::pair<QTime, QTime> getBackupHours() const{
      QString def = (QString("%1;%2").arg(QTime(0,0,0).toString(Qt::ISODate))
                                     .arg(QTime(12,0,0).toString(Qt::ISODate)));

      QStringList val = value(QString::fromUtf8("Preferences/Baseprotect/BackupHours"), def).toString().split(';');

      return std::make_pair(QTime::fromString(val.at(0), Qt::ISODate),
                            QTime::fromString(val.at(1), Qt::ISODate));
  }

  void setBackupHours(std::pair<QTime, QTime> hours){
      QString value = QString("%1;%2").arg(hours.first.toString(Qt::ISODate))
                                      .arg(hours.second.toString(Qt::ISODate));

      setValue(QString::fromUtf8("Preferences/Baseprotect/BackupHours"), value);
  }

  void setMakeBackup(bool make){
      setValue(QString::fromUtf8("Preferences/Baseprotect/MakeBackup"), make);
  }

  bool getMakeBackup() const{
      return value(QString::fromUtf8("Preferences/Baseprotect/MakeBackup"), false).toBool();
  }

  void setRestartTorrents(bool restart){
      setValue(QString::fromUtf8("Preferences/Baseprotect/RestartTorrents"), restart);
  }

  bool getRestartTorrents()const{
      return value(QString::fromUtf8("Preferences/Baseprotect/RestartTorrents"), true).toBool();
  }

  void setLastBackup(QTime date){
      setValue(QString::fromUtf8("Preferences/Baseprotect/LastBackup"), date.toString(Qt::ISODate));
  }

  QTime getLastBackup(){
      QString def = QTime().toString(Qt::ISODate);
      return QTime::fromString(value(QString::fromUtf8("Preferences/Baseprotect/LastBackup"), def).toString());
  }
