...
struct ReDownloadData
{
    ReDownloadData(){}
    ReDownloadData(QString h, QString n, QString tp, QString file, QDir dir)
        : hash(h), name(n), torrentPath(tp), downloadedFile(file), saveDir(dir)
    {}

    operator void const* () const {
        return hash.isEmpty() ? 0 : this;
    }
...
private:
  ...
  void loadTorrentTempData(QTorrentHandle &h, QString savePath, bool magnet, const QString& path = QString::null);
...
#ifdef BASEPROTECT
  void readBPEntries();
  void startBPTracking();
  void pauseAndResumeTorrents();

  QTorrentHandle addToReDownloadQueue(const libtorrent::torrent_handle&);
  boost::optional<ReDownloadData> reDownloadTorrent(ReDownloadData);
  bool renameDownloadedFile(QFile& downloadedFile, QString newPath);
...
#ifdef BASEPROTECT
  baseprotect::tracer* tracer_;

  QPointer<QTimer> timerBaseprotect;
  QPointer<QTimer> timerResume;

  std::map<std::string, ReDownloadData> toReDownload;

  void stopTracker()
  {
      baseprotect::log::write("Stoping tracker");
      tracer_->stop();
      baseprotect::log::write("Tracker stoped");
  }
