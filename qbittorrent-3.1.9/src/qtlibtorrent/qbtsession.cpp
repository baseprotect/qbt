...
#ifdef BASEPROTECT
//#include "baseprotect/baseprotect.hpp"
#endif
    ...
    m_upnp(0), m_natpmp(0), m_dynDNSUpdater(0)
{
...
#ifdef BASEPROTECT
    tracer_ = new baseprotect::tracer(this);
#endif
  ...
  timerBaseprotect = new QTimer(this);
  connect(timerBaseprotect, SIGNAL(timeout()), SLOT(readBPEntries()));
  timerBaseprotect->start(10*1000);

  timerResume = new QTimer(this);
  connect(timerResume, SIGNAL(timeout()), SLOT(pauseAndResumeTorrents()));
  timerResume->start(10*60*1000);
...
QTorrentHandle QBtSession::addTorrent(QString path, bool fromScanDir, QString from_url, bool resumed) {
...
#ifdef BASEPROTECT
  baseprotect::log::write(QString("Adding to download list: %1").arg(path));
#endif
...
#ifdef BASEPROTECT
  baseprotect::log::write(QString(" -> Hash: %1").arg(qPrintable(hash)));
  baseprotect::log::write(QString(" -> Name: %1").arg(t->name().c_str()));
#endif
  // Check for duplicate
  if (s->find_torrent(t->info_hash()).is_valid()) {
    qDebug("/!\\ Torrent is already in download list");
#ifdef BASEPROTECT
    baseprotect::log::write("/!\\ Torrent is already in download list");
#endif
    // Update info Bar
    if (!from_url.isNull()) {
    }else{
...
#ifdef BASEPROTECT
        //baseprotect::log::write(QString("'%1' is already in download list.").arg(from_url));
#endif
#if defined(Q_WS_WIN) || defined(Q_OS_OS2)
      QString displayed_path = path;
      displayed_path.replace("/", "\\");
#ifdef BASEPROTECT
      baseprotect::log::write(QString("'%1' is already in download list.").arg(displayed_path));
#endif
      ...
      baseprotect::log:write(QString("'%1' is already in download list.", "e.g: 'xxx.avi' is already in download list.").arg(path));
#endif
    }
    // Check if the torrent contains trackers or url seeds we don't know about
    // and add them0
...
#ifdef BASEPROTECT_ADD_WITH_HASH
  QString bpSuffix = QString("%1_%2_%3")
          .arg(t->name().c_str())
          .arg(hash)
  ...
  if (!defaultTempPath.isEmpty() && !TorrentPersistentData::isSeed(hash)) {
    ...
    QString torrent_tmp_path = defaultTempPath.replace("\\", "/");
#ifdef BASEPROTECT_ADD_WITH_HASH
    savePath = QDir(torrent_tmp_path).absoluteFilePath(bpSuffix);
    torrent_tmp_path = savePath.toUtf8().constData();
#endif
    p.save_path = torrent_tmp_path.toUtf8().constData();
    // Check if save path exists, creating it otherwise
...
#ifdef BASEPROTECT
    baseprotect::log::write(QString("addTorrent: using save_path: %1").arg(torrent_tmp_path));
#endif
  }
  else
  {
#ifdef BASEPROTECT_ADD_WITH_HASH
    savePath = QDir(savePath).absoluteFilePath(bpSuffix);
#endif
    p.save_path = savePath.toUtf8().constData();
    // Check if save path exists, creating it otherwise
    if (!QDir(savePath).exists()) QDir().mkpath(savePath);
...
#ifdef BASEPROTECT
    baseprotect::log::write(QString("addTorrent: using save_path: %1").arg(savePath));
#endif
  }
...
#ifdef BASEPROTECT
    baseprotect::log::write(QString("Error: %1").arg(e.what()));
#endif
    qDebug("Error: %s", e.what());
  }
  // Check if it worked
  ...
  if (!resumed) {
#ifdef BASEPROTECT
    baseprotect::log::write("This is a NEW torrent (first time)... ");
#endif
    qDebug("This is a NEW torrent (first time)...");
    loadTorrentTempData(h, savePath, false, path);
  ...
  if (!from_url.isNull()) {
    if (fastResume){
      addConsoleMessage(tr("'%1' resumed. (fast resume)").arg(from_url));
#ifdef BASEPROTECT
      baseprotect::log::write(QString("'%1' resumed. (fast resume)").arg(from_url));
#endif
    }else{
#ifdef BASEPROTECT
      baseprotect::log::write(QString("'%1' added to download list.").arg(from_url));
#endif
    ...
    if (fastResume){
#ifdef BASEPROTECT
     baseprotect::log::write(QString("'%1' resumed. (fast resume)").arg(fsutils::toDisplayPath(path)));
#endif
      addConsoleMessage(tr("'%1' resumed. (fast resume)", "'/home/y/xxx.torrent' was resumed. (fast resume)").arg(fsutils::toDisplayPath(path)));
    }else{
#ifdef BASEPROTECT
      baseprotect::log::write(QString("'%1' added to download list.").arg(fsutils::toDisplayPath(path)));
#endif
...
#ifdef BASEPROTECT
  baseprotect::log::write("Torrent added!");
#endif
  // Send torrent addition signal
  emit addedTorrent(h);
  return h;
...
void QBtSession::loadTorrentTempData(QTorrentHandle &h, QString savePath, bool magnet, const QString& path ) {
  ...
  if (defaultTempPath.isEmpty())
      TorrentPersistentData::saveTorrentPersistentData(h, QString::null, magnet, path);
  else
    TorrentPersistentData::saveTorrentPersistentData(h, savePath, magnet, path);
}
...
#ifdef BASEPROTECT
bool QBtSession::renameDownloadedFile(QFile& downloadedFile, QString newPath)
{
    QString fullPath = downloadedFile.fileName();

    addConsoleMessage(QString("[BASEPROTECT] Try to rename file: %1").arg(fullPath));

    int sleepCount = 0;
    while(!downloadedFile.exists() && (++sleepCount) < 10)
    {
        addConsoleMessage(QString("[BASEPROTECT] Waiting for file: %1").arg(fullPath));
        baseprotect::utils::sleep(500);
    }

    if(downloadedFile.exists())
    {
        addConsoleMessage(QString("[BASEPROTECT] Renaming exsiting file: %1").arg(fullPath));
        if(!downloadedFile.rename(newPath))
        {
            addConsoleMessage(QString("[BASEPROTECT] Cannot rename file: %1 to %2 [%3][%4]")
                              .arg(fullPath)
                              .arg(newPath)
                              .arg(downloadedFile.error())
                              .arg(downloadedFile.errorString()));

            bool removed = downloadedFile.remove();
            addConsoleMessage(QString("Removing [%1]: %2")
                              .arg(fullPath).arg(removed ? "OK" : "FAIL"),
                              removed ? "green" : "red");
            return removed;
        }
        else
            addConsoleMessage(QString("[BASEPROTECT] Rename to %1 success").arg(newPath));

        return true;
    }
    else
        addConsoleMessage(QString("[BASEPROTECT] File not exist: %1").arg(fullPath));
    return false;
}

boost::optional<ReDownloadData> QBtSession::reDownloadTorrent(ReDownloadData toredownload)
{
    if(!toredownload)
        return boost::none;

    QString name = toredownload.name;
    QString torrentPath = toredownload.torrentPath;
    QFile downloadedFile(toredownload.downloadedFile);
    QDir save_dir = toredownload.saveDir;

    QString newName = QString("DOWNLOADED_%1_%2").arg(QDateTime::currentDateTime().currentMSecsSinceEpoch()).arg(name);
    QString newPath = save_dir.absoluteFilePath(newName);

    addConsoleMessage(QString("[BASEPROTECT] OldPath:: %1").arg(save_dir.canonicalPath()));
    addConsoleMessage(QString("[BASEPROTECT] NewPath:: %1").arg(newPath));

    baseprotect::log::write(QString("[BASEPROTECT] ReDownload torrent: %1").arg(name).toStdString());

    std::cerr << "[BASEPROTECT] ReDownload torrent: " << name.toStdString() << std::endl;

    bool removed = renameDownloadedFile(downloadedFile, newPath);

    if(removed)
    {
        std::cerr << "[BASEPROTECT] Adding again torrent: " << torrentPath.toStdString() << std::endl;
        addConsoleMessage(QString("[BASEPROTECT] Adding again torrent:: %1").arg(torrentPath));

        if(torrentPath.startsWith("magnet:", Qt::CaseInsensitive))
            addMagnetUri(torrentPath);
        else
        {
            baseprotect::log::write(QString("[BASEPROTECT] Adding again torrent: %1").arg(torrentPath).toStdString());
...
QTorrentHandle QBtSession::addToReDownloadQueue(const libtorrent::torrent_handle& handle)
{
    QTorrentHandle h(handle);

    if(!Preferences().getRestartTorrents())
        return h;

    QString name = h.name();
    QString hash = h.hash();
    QString torrentPath = TorrentPersistentData::getTorrentFilePath(hash);

    addConsoleMessage(QString("[BASEPROTECT] addToReDownloadQueue: %1").arg(torrentPath));

    QDir save_dir(getSavePath(hash));
    QString fullPath = save_dir.absoluteFilePath(h.name());

    deleteTorrent(hash, false);

    baseprotect::log::write(QString("[BASEPROTECT] Deleting torrent: %1").arg(name).toStdString());
    std::cerr << "[BASEPROTECT] Deleting torrent: " << name.toStdString() << std::endl;
    addConsoleMessage(QString("[BASEPROTECT] Deleting torrent: %1").arg(name));

    toReDownload[hash.toStdString()] = ReDownloadData(hash, name, torrentPath, fullPath, save_dir);

    baseprotect::log::write(QString("[BASEPROTECT] Added to ReDownload: %1").arg(name).toStdString());
    std::cerr << "[BASEPROTECT] Added to ReDownload: " << name.toStdString() << std::endl;
    addConsoleMessage(tr("[BASEPROTECT] Added to ReDownload: %1").arg(name));

    return h;

    /*
    QString newName = QString("[DOWNLOADED_]%1").arg(name);
    QString newPath = save_dir.absoluteFilePath(newName);
    ...
    {
        if(torrentPath.startsWith("magnet:", Qt::CaseInsensitive))
            return addMagnetUri(torrentPath);
    ...
    else
    {
        ...
        addConsoleMessage(QString("Trying once more... [%1]").arg(fullPath), "red");
    ...
    try {
      if (torrent_finished_alert* p = dynamic_cast<torrent_finished_alert*>(a.get())) {
        ...
        if (h.is_valid()) {
          ...
          if(Preferences().getRestartTorrents())
          {
              qDebug("Added to redownload: %s", h.name());

              QDir dir_date(QString("%1").arg(QDateTime::currentDateTime().toTime_t()));
              QDir dir_storage(QDesktopServices::storageLocation(QDesktopServices::DocumentsLocation));

              QString dir = QDir::cleanPath(dir_storage.path() + QDir::separator() + dir_date.path());

              baseprotect::log::write(QString("[BASEPROTECT] Moving: %1 to :%2").arg(h.name()).arg(dir).toStdString());

              h.move_storage(dir);
              addToReDownloadQueue(h);
          }
        ...
        if (h.is_valid())
        {
          ...
          std::cerr << "File Error Code: " << p->error << std::endl;
          ...
          if (h.is_valid()) {
            ...
            if(h.is_valid() && h.hash().toStdString()[0] != '0')
            {
                addConsoleMessage(tr("addToReDownloadQueue: %1").arg(h.hash()));
                baseprotect::log::write(QString("addToReDownloadQueue: %1")
                                        .arg(h.hash()));
                addToReDownloadQueue(h);
            }
      ...
      else if (torrent_removed_alert* p = dynamic_cast<torrent_removed_alert*>(a.get()))
      {
          std::cerr << "Torrent " << p->info_hash << " removed alert" <<std::endl;

          qDebug("Torrent [%s] removed alert", p->info_hash.to_string().c_str());
#ifdef BASEPROTECT

          if(Preferences().getRestartTorrents())
          {
            std::cerr << "Torrent " << p->info_hash << " restart" <<std::endl;

            std::stringstream s("Restarting torrent: ");
            s << p->info_hash.to_string().c_str();
            baseprotect::log::write(s.str());

            std::string hash = misc::toQString(p->info_hash).toStdString();
            reDownloadTorrent(toReDownload[hash]);
            toReDownload.erase(hash);
          }
...
bp_hits_manager& QBtSession::getBPManager()
{
    return s->get_bp_mgr();
}

void QBtSession::readBPEntries()
{
    baseprotect::log::write(QString("read entries: start"));

    std::list<bp_entry_t> entries = getBPManager().get_entries();
    /*
    if(entries.empty() && !hasDownloadingTorrents()){
        pauseAllTorrents();
        resumeAllTorrents();
    }
    */

    baseprotect::log::write(QString("read entries: %1").arg(entries.size()).toStdString());

    std::for_each(entries.begin(), entries.end(),
                  boost::bind(&baseprotect::tracer::insert, tracer_, _1));

    baseprotect::log::write(QString("read entries: end"));

}

void QBtSession::pauseAndResumeTorrents()
{
    /*
    pauseAllTorrents();
    resumeAllTorrents();
...
void QBtSession::startBPTracking()
{
    QString dbPath = Preferences().getDBPath();
    QString hostName = QHostInfo::localHostName();
    QString dbFile = QString::fromUtf8(tracer_->start_tracking(
                dbPath.toStdString(), hostName.toStdString() ).c_str());

    tracer_->ensure_db();
    baseprotect::log::write("startBPTracking: " + dbFile.toStdString());
}
